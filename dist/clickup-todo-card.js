...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.19  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class At extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1}static async getConfigElement(){return await Promise.resolve().then(function(){return St}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return V`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return V`
        ...
        `;this._tasks=yt(t);const e=$t(function(t,e){let i=[...t];if(!e.filters)return i;if(e.filters.status?.length&&(i=i.filter(t=>{const i=t.clickup_status?.status;return i&&e.filters.status.includes(i)})),e.filters.priority?.length&&(i=i.filter(t=>{const i=null===t.priority?null:t.priority;return e.filters.priority.includes(i)})),e.filters.tags?.length&&(i=i.filter(t=>!!t.tags?.length&&t.tags.some(t=>e.filters.tags.includes(t.name)))),e.filters.assignees?.length&&(i=i.filter(t=>!!t.assignees?.length&&t.assignees.some(t=>e.filters.assignees.includes(t.id.toString())))),e.filters.due_date_range){const{start:t,end:s}=e.filters.due_date_range;i=i.filter(e=>{if(!e.due)return!1;const i=new Date(e.due).getTime();return!(t&&i<new Date(t).getTime())&&!(s&&i>new Date(s).getTime())})}return i}(this._tasks,this._config),this._config),i=wt(e,this._config.group_by||"none",this._config.group_field_id);return V`
        <ha-card>
      ...
      `}}_renderHeader(t){if(this._config.hide_header)return V``;const e=this._config.title||t.attributes.friendly_name||"Tasks",i="unavailable"===t.state,s=!1!==this._config.show_task_count;return V`
      <div class="card-header">
        <div class="name">${e}</div>
        ${s?V`<div class="task-count">${this._tasks.length}</div>`:""}
      </div>
    ...
    `:V`
      ...
      `}_renderTask(t){const e=function(t){if(!t.due)return!1;const e=new Date;return new Date(t.due)<e&&"completed"!==t.status}(t),i="completed"===t.status;return V`
    ...
    `:V``}_renderStatus(t){if(!this._config.show_status||!t.clickup_status)return V``;const e=t.clickup_status.color||"var(--primary-color)";return V`
    ...
    `}async _toggleTask(t){const e="completed"===t.status?"needs_action":"completed";try{await this.hass.callService("todo","update_item",{entity_id:this._config.entity,item:t.uid,status:e})}catch(t){console.error("Error toggling task:",t)}}async _addItem(){const t=this.shadowRoot?.querySelector("ha-textfield");if(!t||!t.value?.trim())return void(this._showAddDialog=!0);const e=t.value.trim();try{await this.hass.callService("todo","add_item",{item:e},{entity_id:this._config.entity}),t.value="",t.blur()}catch(t){console.error("Error adding item:",t)}}_handleAddKeyPress(t){"Enter"===t.key&&this._addItem()}_openAddDialog(){this._showAddDialog=!0}_openEditDialog(t){this._editingTask=t}_renderAddDialog(){return V`
      <ha-dialog
        open
