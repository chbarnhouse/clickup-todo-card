...
`;async function Ot(t,e,i){try{return e.clickup_id?(await t.callService("clickup","update_task_status",{task_id:e.clickup_id,status:i}),{success:!0}):{success:!1,error:"Task does not have a ClickUp ID"}}catch(t){return console.error("Error updating task status:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}let Pt=class extends nt{constructor(){super(...arguments),this.value="",this.editing=!1,this.placeholder="Click to edit",this.required=!1,this.multiline=!1,this._tempValue="",this._error=""}static{this.styles=r`
  ...
  `}render(){return j`
    ...
    `}_openDialog(){this._isOpen=!0}_closeDialog(){this._isOpen=!1}_selectStatus(t){const e=this.value;(e?.name||e?.status||"")!==(t.name||t.status||"")&&(this.value=t,this.dispatchEvent(new CustomEvent("value-changed",{detail:{value:t,oldValue:e},bubbles:!0,composed:!0}))),this._isOpen=!1}};t([ht({type:Object})],Lt.prototype,"value",void 0),t([ht({type:Array})],Lt.prototype,"options",void 0),t([ht({type:Boolean})],Lt.prototype,"compact",void 0),t([ut()],Lt.prototype,"_isOpen",void 0),Lt=t([ct("editable-status")],Lt),console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 2.7.8  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class Rt extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...yt},this._tasks=[],this._displayedTasks=[],this._editingTask=null,this._showAddDialog=!1,this._selectedTasks=new Set,this._selectionMode=!1,this._draggedTask=null,this._dragOverTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Bt}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...yt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...yt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var a=e.get("hass");return!a||a.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}_calculatePillWidth(t){if(!this._config.show_status||!t||0===t.length)return 100;let e="";for(const i of t)if(i.clickup_status?.status){const t=i.clickup_status.status;t.length>e.length&&(e=t)}if(!e)return 100;const i=this._config.compact_mode,a=i?8.5:9,o=(i?22:24)+(i?3:4)+Math.ceil(e.length*a)+(i?13:16);return 5*Math.ceil((o+10)/5)}render(){try{if(!this._config||!this.hass)return j`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return j`
        ...
        `;this._tasks=$t(t);let e=this._tasks;if("custom"===this._config.sort_by){const t=function(t){try{const e=Dt(t),i=localStorage.getItem(e);return i?JSON.parse(i):null}catch(t){return console.error("Error loading custom order:",t),null}}(this._config.entity);t&&(e=function(t,e){const i=new Map(t.map(t=>[t.uid,t])),a=[],o=new Set;for(const t of e){const e=i.get(t);e&&(a.push(e),o.add(t))}for(const e of t)o.has(e.uid)||a.push(e);return a}(this._tasks,t))}const i=wt(function(t,e){let i=[...t];if(!e.filters)return i;if(e.filters.status?.length&&(i=i.filter(t=>{const i=t.clickup_status?.status;return i&&e.filters.status.includes(i)})),e.filters.priority?.length&&(i=i.filter(t=>{const i=null===t.priority?null:t.priority;return e.filters.priority.includes(i)})),e.filters.tags?.length&&(i=i.filter(t=>!!t.tags?.length&&t.tags.some(t=>e.filters.tags.includes(t.name)))),e.filters.assignees?.length&&(i=i.filter(t=>!!t.assignees?.length&&t.assignees.some(t=>e.filters.assignees.includes(t.id.toString())))),e.filters.due_date_range){const{start:t,end:a}=e.filters.due_date_range;i=i.filter(e=>{if(!e.due)return!1;const i=new Date(e.due).getTime();return!(t&&i<new Date(t).getTime())&&!(a&&i>new Date(a).getTime())})}return i}(e,this._config),this._config);this._displayedTasks=i;const a=this._calculatePillWidth(i),o=St(i,this._config.group_by||"none",this._config.group_field_id),s=!1!==this._config.add_button_overlay,r=this._config.add_button_position||"bottom-right",n=!s&&r.startsWith("top"),l=!s&&r.startsWith("bottom");return j`
        <ha-card>
          ...
          >
            ${1===o.size&&o.has("all")?this._renderTaskList(o.get("all")):this._renderGroupedTasks(o)}
          </div>
    ...
    `:j`
      ...
      `}_renderTask(t){const e=function(t){if(!t.due)return!1;const e=new Date;return new Date(t.due)<e&&"completed"!==t.status}(t),i="completed"===t.status,a=this._config.show_status&&t.clickup_status,o=this._selectedTasks.has(t.uid),s=this._dragOverTask===t.uid;return j`
      <div
        class="task-item ${i?"completed":""} ${e?"overdue":""} ${o?"selected":""} ${s?"drag-over":""}"
        draggable="true"
    ...
    `}_renderTaskMetadata(t){const e=this._getMetadataGrid();if(!e.fields||0===e.fields.length)return j``;const i=e.columns||xt,a=e.gap||kt,o=this._config.compact_mode?"4px 6px":a;return j`
      <div
        class="task-metadata"
        style="
          grid-template-columns: ${this._config.compact_mode?i.replace("150px","140px"):i};
          gap: ${this._config.compact_mode?o:a};
        "
      >
    ...
    `}_addMetadataField(){if(!this._config.metadata_grid)return;const t={...this._config,metadata_grid:{...this._config.metadata_grid,fields:[...this._config.metadata_grid.fields||[],{type:"location"}]}};_t(this,"config-changed",{config:t})}_removeMetadataField(t){if(!this._config.metadata_grid)return;const e=[...this._config.metadata_grid.fields||[]];e.splice(t,1);const i={...this._config,metadata_grid:{...this._config.metadata_grid,fields:e}};_t(this,"config-changed",{config:i})}_moveMetadataField(t,e){if(!this._config.metadata_grid)return;const i=[...this._config.metadata_grid.fields||[]],a=t+e;if(a<0||a>=i.length)return;[i[t],i[a]]=[i[a],i[t]];const o={...this._config,metadata_grid:{...this._config.metadata_grid,fields:i}};_t(this,"config-changed",{config:o})}_metadataFieldPropertyChanged(t,e,i){if(!this._config.metadata_grid)return;const a=[...this._config.metadata_grid.fields||[]];if("type"===e)a[t]={...a[t],type:i};else if("span"===e)if(""===i){const{span:e,...i}=a[t];a[t]=i}else a[t]={...a[t],span:i};const o={...this._config,metadata_grid:{...this._config.metadata_grid,fields:a}};_t(this,"config-changed",{config:o})}static get styles(){return r`
      .card-config {
        display: flex;
        flex-direction: column;
        height: 100%;
