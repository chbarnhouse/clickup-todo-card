...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.34  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class At extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1,this._statusDropdownTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Ct}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return B`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return B`
        ...
        <ha-card>
          ...
          ${r?B`<div class="button-container">${this._renderFloatingAddButton(t)}</div>`:""}
      ...
      `}}_renderHeader(t){if(this._config.hide_header)return B``;const e=this._config.title||t.attributes.friendly_name||"Tasks",i=!1!==this._config.show_task_count,s=!0!==this._config.hide_title;if(!s&&!i)return B``;const o=this._tasks.length;return B`
      <div class="card-header">
        ${s?B`<div class="name">${e}</div>`:""}
        ${i?B`<div class="task-count ${o<10?"single-digit":""}">${o}</div>`:""}
      </div>
    ...
    `:B`
      ...
      `}_renderTask(t){const e=function(t){if(!t.due)return!1;const e=new Date;return new Date(t.due)<e&&"completed"!==t.status}(t),i="completed"===t.status,s=this._config.show_status&&t.clickup_status,o=this._statusDropdownTask===t.uid;return B`
    ...
    `}_renderDates(t){const e=this._config.show_start_date&&t.start_date,i=this._config.show_due_date&&t.due;return e||i?B`
      <div class="task-dates">
        ...
        ${i?B`
          <span class="date-item due-date ${function(t){if(!t)return"";let e;if(e="number"==typeof t||"string"==typeof t?new Date(t):t,isNaN(e.getTime()))return"";const i=new Date,s=new Date(i.getFullYear(),i.getMonth(),i.getDate()),o=new Date(e.getFullYear(),e.getMonth(),e.getDate()).getTime()-s.getTime(),a=Math.ceil(o/864e5);return a<0?"overdue":0===a?"today":1===a?"tomorrow":a<=7?"this-week":"future"}(t.due)}">
    ...
    `}async _toggleTask(t){const e="completed"===t.status?"needs_action":"completed";try{await this.hass.callService("todo","update_item",{entity_id:this._config.entity,item:t.uid,status:e})}catch(t){console.error("Error toggling task:",t)}}_toggleStatusDropdown(t){this._statusDropdownTask=this._statusDropdownTask===t?null:t}_renderStatusDropdown(t){return B`
      <div class="status-dropdown" @click=${t=>t.stopPropagation()}>
        ${[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}].map(e=>B`
          <div
            class="status-option"
    ...
    `}async _changeTaskStatus(t,e){try{this._statusDropdownTask=null,await this.hass.callService("clickup","update_task_status",{task_id:t.clickup_id,status:e})}catch(t){console.error("Error changing task status:",t)}}_openAddDialog(){this._showAddDialog=!0}_openEditDialog(t){this._editingTask=t}_renderAddDialog(){return B`
      <ha-dialog
        open
      ...
      >
        <div class="dialog-content">
          ...
          <ha-select
            ...
            id="add-status"
          >
            ${[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}].map(t=>B`
    ...
    `}async _submitAddTask(){const t=this.shadowRoot?.querySelector("#add-summary")?.value?.trim();if(!t)return;const e=this.shadowRoot?.querySelector("#add-description")?.value?.trim(),i=this.shadowRoot?.querySelector("#add-status")?.value,s=this.shadowRoot?.querySelector("#add-start-date")?.value,o=this.shadowRoot?.querySelector("#add-due-date")?.value,a=this.shadowRoot?.querySelector("#add-priority")?.value;try{const r={item:t};e&&(r.description=e),o&&(r.due=o),await this.hass.callService("todo","add_item",r,{entity_id:this._config.entity}),(i||a||s)&&setTimeout(async()=>{try{const e=this.hass.states[this._config.entity],o=(e?.attributes?.clickup_tasks||[]).find(e=>e.summary===t);o&&o.clickup_id&&(i&&await this.hass.callService("clickup","update_task_status",{task_id:o.clickup_id,status:i}),a&&await this.hass.callService("clickup","update_task_priority",{task_id:o.clickup_id,priority:parseInt(a)}),s&&await this.hass.callService("clickup","update_task_start_date",{task_id:o.clickup_id,start_date:new Date(s).getTime()}))}catch(t){console.error("Error updating task details:",t)}},1e3),this._showAddDialog=!1}catch(t){console.error("Error adding task:",t)}}async _submitEditTask(){if(!this._editingTask)return;const t=this.shadowRoot?.querySelector("#edit-summary")?.value?.trim();if(!t)return;const e=this.shadowRoot?.querySelector("#edit-description")?.value?.trim(),i=this.shadowRoot?.querySelector("#edit-status")?.value,s=this.shadowRoot?.querySelector("#edit-start-date")?.value,o=this.shadowRoot?.querySelector("#edit-due-date")?.value,a=this.shadowRoot?.querySelector("#edit-priority")?.value;try{const r=this._editingTask.clickup_id,n={entity_id:this._config.entity,item:this._editingTask.uid,rename:t};if(void 0!==e&&(n.description=e||null),o&&(n.due=o),await this.hass.callService("todo","update_item",n),r){i&&i!==this._editingTask.clickup_status?.status&&await this.hass.callService("clickup","update_task_status",{task_id:r,status:i});const t=a?parseInt(a):null;t!==this._editingTask.priority&&await this.hass.callService("clickup","update_task_priority",{task_id:r,priority:t});s!==(this._editingTask.start_date?new Date("number"==typeof this._editingTask.start_date?this._editingTask.start_date:parseInt(this._editingTask.start_date)).toISOString().split("T")[0]:"")&&await this.hass.callService("clickup","update_task_start_date",{task_id:r,start_date:s?new Date(s).getTime():null})}this._editingTask=null}catch(t){console.error("Error updating task:",t)}}async _deleteTask(t){if(confirm(`Delete task "${t.summary}"?`))try{await this.hass.callService("todo","remove_item",{entity_id:this._config.entity,item:t.uid}),this._editingTask=null}catch(t){console.error("Error deleting task:",t)}}static get styles(){return kt}}t([ht({attribute:!1})],At.prototype,"hass",void 0),t([ut()],At.prototype,"_config",void 0),t([ut()],At.prototype,"_tasks",void 0),t([ut()],At.prototype,"_editingTask",void 0),t([ut()],At.prototype,"_showAddDialog",void 0),t([ut()],At.prototype,"_statusDropdownTask",void 0),customElements.get("clickup-todo-card")||(customElements.define("clickup-todo-card",At),console.log("ClickUp Todo Card: Manually registered custom element")),window.customCards=window.customCards||[],window.customCards.push({type:"clickup-todo-card",name:"ClickUp Todo Card",description:"Enhanced todo card with ClickUp custom fields and filters",preview:!0,documentationURL:"https://github.com/chbarnhouse/clickup-todo-card"}),console.log("ClickUp Todo Card: Registered in customCards array");class St extends nt{constructor(){super(),this._config={type:"custom:clickup-todo-card",entity:""},this._tasks=[],console.log("ClickUp Card Editor: Constructor called")}connectedCallback(){super.connectedCallback(),console.log("ClickUp Card Editor: Connected to DOM"),console.log("ClickUp Card Editor: _handleSelectChange method exists?","function"==typeof this._handleSelectChange)}setConfig(t){this._config={type:"custom:clickup-todo-card",entity:"",...t},this._loadEntityData()}_loadEntityData(){if(!this.hass||!this._config.entity)return;const t=this.hass.states[this._config.entity];t&&(this._tasks=yt(t))}render(){try{if(!this.hass)return B`<div>Loading hass...</div>`;if(!this._config)return B`<div>Loading config...</div>`;const t=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.clickup_status?.status&&e.set(t.clickup_status.status,t.clickup_status.status)}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],e=this._tasks.length>0?function(t){const e=new Set;return t.forEach(t=>{t.tags?.forEach(t=>{t.name&&e.add(t.name)})}),Array.from(e).map(t=>({value:t,label:t})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],i=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.assignees?.forEach(t=>{t.id&&t.username&&e.set(t.id.toString(),t.username)})}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],s=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.custom_fields?.forEach(t=>{t.id&&t.name&&e.set(t.id,t.name)})}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[];return B`
        ...
        <div class="config-section">
          ...
          <div style="display: flex; flex-direction: column; gap: 4px;">
            ...
            <select
              ...
              @change=${t=>{console.log("Native select change event fired!");const e=t.target;console.log("Selected value:",e.value);const i={...this._config,add_button_position:e.value};ft(this,"config-changed",{config:i}),this._config=i,this.requestUpdate()}}
            >
