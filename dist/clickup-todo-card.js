...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.20  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class At extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1}static async getConfigElement(){return await Promise.resolve().then(function(){return St}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return I`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return I`
      ...
      `}}_renderHeader(t){if(this._config.hide_header)return I``;const e=this._config.title||t.attributes.friendly_name||"Tasks",i="unavailable"===t.state,s=!1!==this._config.show_task_count,a=!0!==this._config.hide_title;return I`
      ${a||s?I`
        <div class="card-header">
          ${a?I`<div class="name">${e}</div>`:""}
          ${s?I`<div class="task-count">${this._tasks.length}</div>`:""}
        </div>
      `:""}
    ...
    `:I``}_renderStatus(t){if(!this._config.show_status||!t.clickup_status)return I``;const e=t.clickup_status.color||"var(--primary-color)";return I`
    ...
    `}_renderEditDialog(){if(!this._editingTask)return I``;const t=this._editingTask,e=t.start_date?new Date("number"==typeof t.start_date?t.start_date:parseInt(t.start_date)).toISOString().split("T")[0]:"",i=t.due?new Date(t.due).toISOString().split("T")[0]:"";return I`
      <ha-dialog
        open
