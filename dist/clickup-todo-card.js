...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.65  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class St extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1,this._statusDropdownTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Et}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return V`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return V`
          <ha-card>
            <div class="warning">Please configure an entity in the card editor</div>
          </ha-card>
        `;const t=this.hass.states[this._config.entity];if(!t)return V`
          <ha-card>
            <div class="warning">Entity not found: ${this._config.entity}</div>
          </ha-card>
        `;this._tasks=bt(t);const e=xt(function(t,e){let i=[...t];if(!e.filters)return i;if(e.filters.status?.length&&(i=i.filter(t=>{const i=t.clickup_status?.status;return i&&e.filters.status.includes(i)})),e.filters.priority?.length&&(i=i.filter(t=>{const i=null===t.priority?null:t.priority;return e.filters.priority.includes(i)})),e.filters.tags?.length&&(i=i.filter(t=>!!t.tags?.length&&t.tags.some(t=>e.filters.tags.includes(t.name)))),e.filters.assignees?.length&&(i=i.filter(t=>!!t.assignees?.length&&t.assignees.some(t=>e.filters.assignees.includes(t.id.toString())))),e.filters.due_date_range){const{start:t,end:s}=e.filters.due_date_range;i=i.filter(e=>{if(!e.due)return!1;const i=new Date(e.due).getTime();return!(t&&i<new Date(t).getTime())&&!(s&&i>new Date(s).getTime())})}return i}(this._tasks,this._config),this._config),i=kt(e,this._config.group_by||"none",this._config.group_field_id),s=!1!==this._config.add_button_overlay,a=this._config.add_button_position||"bottom-right",o=!s&&a.startsWith("top"),r=!s&&a.startsWith("bottom");return V`
        <ha-card>
          ${this._renderHeader(t)}
          ${o?V`<div class="button-container">${this._renderFloatingAddButton(t)}</div>`:""}
          ...
          ${r?V`<div class="button-container">${this._renderFloatingAddButton(t)}</div>`:""}
          ${s?this._renderFloatingAddButton(t):""}
        </ha-card>
        ${this._showAddDialog?this._renderAddDialog():""}
        ${this._editingTask?this._renderEditDialog():""}
      `}catch(t){return console.error("Error rendering ClickUp Todo Card:",t),V`
        <ha-card>
          <div class="warning">Error rendering card. Check console for details.</div>
        </ha-card>
      `}}_renderHeader(t){if(this._config.hide_header)return V``;const e=this._config.title||t.attributes.friendly_name||"Tasks",i=!1!==this._config.show_task_count,s=!0!==this._config.hide_title;if(!s&&!i)return V``;const a=this._tasks.length;return V`
      <div class="card-header">
        ${s?V`<div class="name">${e}</div>`:""}
        ${i?V`<div class="task-count ${a<10?"single-digit":""}">${a}</div>`:""}
      </div>
    `}_renderFloatingAddButton(t){const e="unavailable"===t.state,i=void 0!==this._config.add_button_text?this._config.add_button_text:"Add Task",s=this._config.add_button_position||"bottom-right",a=!1!==this._config.add_button_overlay;return V`
      <button
        class="floating-add-button ${s} ${""===i?"icon-only":""} ${a?"overlay":"non-overlay"}"
        ?disabled=${e}
    ...
    `}_renderGroupedTasks(t){const e=Array.from(t.entries());return V`
      ${e.map(([t,e])=>V`
      ...
      `)}
    `}_renderTaskList(t){return t&&0!==t.length?V`
      <div class="tasks">
    ...
    `:V`
      ...
      `}_renderTask(t){const e=function(t){if(!t.due)return!1;const e=new Date;return new Date(t.due)<e&&"completed"!==t.status}(t),i="completed"===t.status,s=this._config.show_status&&t.clickup_status,a=this._statusDropdownTask===t.uid;return V`
      <div class="task-item ${i?"completed":""} ${e?"overdue":""}">
        ${s?V`
          <div class="task-status-wrapper" @click=${e=>{e.stopPropagation(),this._toggleStatusDropdown(t.uid)}}>
            ...
            ></ha-checkbox>
            ${a?this._renderStatusDropdown(t):""}
          </div>
        `:V`
    ...
    `}_renderPriority(t){if(!this._config.show_priority||null===t.priority||void 0===t.priority)return V``;const e=t.priority;return V`
      <ha-icon
        class="priority-icon"
        icon="${mt[e]||mt.null}"
        style="color: ${_t[e]||_t.null}"
      ></ha-icon>
    `}_renderDates(t){const e=this._config.show_start_date&&t.start_date,i=this._config.show_due_date&&t.due;return e||i?V`
    ...
    `:V``}_renderStatus(t){if(!t.clickup_status)return V``;const e=t.clickup_status.color||"var(--primary-color)";return V`
    ...
    `}_renderTags(t){return this._config.show_tags&&t.tags&&0!==t.tags.length?V`
      <div class="task-tags">
        ${t.tags.map(t=>V`
          <span
            class="tag"
    ...
    `:V``}_renderAssignees(t){return this._config.show_assignees&&t.assignees&&0!==t.assignees.length?V`
      <div class="task-assignees">
        ${t.assignees.map(t=>V`
          <div
            ...
            title="${t.username}"
          >
            ${t.profilePicture?V`<img src="${t.profilePicture}" alt="${t.username}" />`:V`<span>${function(t){if(!t)return"?";const e=t.trim().split(/\s+/);return 1===e.length?e[0].charAt(0).toUpperCase():(e[0].charAt(0)+e[e.length-1].charAt(0)).toUpperCase()}(t.username)}</span>`}
          </div>
        `)}
      </div>
    `:V``}_renderCustomFields(t){if(!this._config.show_custom_fields||!t.custom_fields||0===t.custom_fields.length)return V``;let e=t.custom_fields;return this._config.visible_custom_fields&&this._config.visible_custom_fields.length>0&&(e=e.filter(t=>this._config.visible_custom_fields.includes(t.id))),0===e.length?V``:V`
      <div class="custom-fields">
        ${e.map(t=>V`
          <div class="custom-field">
            <span class="field-name">${t.name}:</span>
            <span class="field-value">${function(t){if(null===t.value||void 0===t.value||""===t.value)return"-";switch(t.type){case"text":case"email":case"phone":case"url":default:return String(t.value);case"number":return Number(t.value).toLocaleString();case"currency":const e=t.type_config?.currency_type||"USD";return new Intl.NumberFormat(void 0,{style:"currency",currency:e}).format(Number(t.value));case"date":return wt(t.value);case"checkbox":return t.value?"✓":"✗";case"drop_down":if(t.type_config?.options){const e=t.type_config.options.find(e=>e.id===t.value||e.name===t.value);return e?.name||String(t.value)}return String(t.value);case"labels":return Array.isArray(t.value)?t.value.join(", "):String(t.value);case"rating":const i=Number(t.value);return"★".repeat(i)+"☆".repeat(5-i);case"location":return"object"==typeof t.value&&t.value.location?t.value.location:String(t.value)}}(t)}</span>
          </div>
        `)}
      </div>
    `}_renderTaskLocation(t){if(!this._config.show_task_locations)return V``;const e=[];return(t.space_info?.name||t.space?.name)&&e.push(t.space_info?.name||t.space.name),t.folder_info?.name&&e.push(t.folder_info.name),(t.list_info?.name||t.list?.name)&&e.push(t.list_info?.name||t.list.name),0===e.length?V``:V`
      <div class="task-location">
        <ha-icon icon="mdi:folder-outline"></ha-icon>
        <span>${e.join(" / ")}</span>
      </div>
    `}async _toggleTask(t){const e="completed"===t.status?"needs_action":"completed";try{await this.hass.callService("todo","update_item",{entity_id:this._config.entity,item:t.uid,status:e})}catch(t){console.error("Error toggling task:",t)}}_toggleStatusDropdown(t){this._statusDropdownTask=this._statusDropdownTask===t?null:t}_renderStatusDropdown(t){const e=t.list_info?.statuses||[],i=this.hass.states[this._config.entity],s=i?.attributes?.available_statuses||[];let a=[];if(e.length>0)a=e.map(t=>({name:t.status,color:t.color||"#d3d3d3",type:t.type}));else if(s.length>0)a=s.map(t=>({name:t.status,color:t.color||"#d3d3d3",type:t.type}));else{const t=yt(this._tasks);a=t.length>0?t:[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}]}return V`
    ...
    `}async _changeTaskStatus(t,e){try{this._statusDropdownTask=null,await this.hass.callService("clickup","update_task_status",{task_id:t.clickup_id,status:e})}catch(t){console.error("Error changing task status:",t)}}_openAddDialog(){this._showAddDialog=!0}_openEditDialog(t){this._editingTask=t}_renderAddDialog(){const t=yt(this._tasks),e=t.length>0?t:[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}];return V`
      <ha-dialog
        open
    ...
    `}_renderEditDialog(){if(!this._editingTask)return V``;const t=this._editingTask,e=t.start_date?new Date("number"==typeof t.start_date?t.start_date:parseInt(t.start_date)).toISOString().split("T")[0]:"",i=t.due?new Date(t.due).toISOString().split("T")[0]:"",s=yt(this._tasks),a=s.length>0?s:[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}];return V`
      <ha-dialog
        open
    ...
    `}async _submitAddTask(){const t=this.shadowRoot?.querySelector("#add-summary")?.value?.trim();if(!t)return;const e=this.shadowRoot?.querySelector("#add-description")?.value?.trim(),i=this.shadowRoot?.querySelector("#add-status")?.value,s=this.shadowRoot?.querySelector("#add-start-date")?.value,a=this.shadowRoot?.querySelector("#add-due-date")?.value,o=this.shadowRoot?.querySelector("#add-priority")?.value;try{const r={item:t};e&&(r.description=e),a&&(r.due=a),await this.hass.callService("todo","add_item",r,{entity_id:this._config.entity}),(i||o||s)&&setTimeout(async()=>{try{const e=this.hass.states[this._config.entity],a=(e?.attributes?.clickup_tasks||[]).find(e=>e.summary===t);a&&a.clickup_id&&(i&&await this.hass.callService("clickup","update_task_status",{task_id:a.clickup_id,status:i}),o&&await this.hass.callService("clickup","update_task_priority",{task_id:a.clickup_id,priority:parseInt(o)}),s&&await this.hass.callService("clickup","update_task_start_date",{task_id:a.clickup_id,start_date:new Date(s).getTime()}))}catch(t){console.error("Error updating task details:",t)}},1e3),this._showAddDialog=!1}catch(t){console.error("Error adding task:",t)}}async _submitEditTask(){if(!this._editingTask)return;const t=this.shadowRoot?.querySelector("#edit-summary")?.value?.trim();if(!t)return;const e=this.shadowRoot?.querySelector("#edit-description")?.value?.trim(),i=this.shadowRoot?.querySelector("#edit-status")?.value,s=this.shadowRoot?.querySelector("#edit-start-date")?.value,a=this.shadowRoot?.querySelector("#edit-due-date")?.value,o=this.shadowRoot?.querySelector("#edit-priority")?.value;try{const r=this._editingTask.clickup_id,n={entity_id:this._config.entity,item:this._editingTask.uid,rename:t};if(void 0!==e&&(n.description=e||null),a&&(n.due=a),await this.hass.callService("todo","update_item",n),r){i&&i!==this._editingTask.clickup_status?.status&&await this.hass.callService("clickup","update_task_status",{task_id:r,status:i});const t=o?parseInt(o):null;t!==this._editingTask.priority&&await this.hass.callService("clickup","update_task_priority",{task_id:r,priority:t});s!==(this._editingTask.start_date?new Date("number"==typeof this._editingTask.start_date?this._editingTask.start_date:parseInt(this._editingTask.start_date)).toISOString().split("T")[0]:"")&&await this.hass.callService("clickup","update_task_start_date",{task_id:r,start_date:s?new Date(s).getTime():null})}this._editingTask=null}catch(t){console.error("Error updating task:",t)}}async _deleteTask(t){if(confirm(`Delete task "${t.summary}"?`))try{await this.hass.callService("todo","remove_item",{entity_id:this._config.entity,item:t.uid}),this._editingTask=null}catch(t){console.error("Error deleting task:",t)}}static get styles(){return At}}t([ht({attribute:!1})],St.prototype,"hass",void 0),t([ut()],St.prototype,"_config",void 0),t([ut()],St.prototype,"_tasks",void 0),t([ut()],St.prototype,"_editingTask",void 0),t([ut()],St.prototype,"_showAddDialog",void 0),t([ut()],St.prototype,"_statusDropdownTask",void 0),customElements.get("clickup-todo-card")||(customElements.define("clickup-todo-card",St),console.log("ClickUp Todo Card: Manually registered custom element")),window.customCards=window.customCards||[],window.customCards.push({type:"clickup-todo-card",name:"ClickUp Todo Card",description:"Enhanced todo card with ClickUp custom fields and filters",preview:!0,documentationURL:"https://github.com/chbarnhouse/clickup-todo-card"}),console.log("ClickUp Todo Card: Registered in customCards array");class Ct extends nt{constructor(){super(),this._config={type:"custom:clickup-todo-card",entity:""},this._tasks=[],console.log("ClickUp Card Editor: Constructor called")}connectedCallback(){super.connectedCallback()}setConfig(t){this._config={type:"custom:clickup-todo-card",entity:"",...t},this._loadEntityData()}_loadEntityData(){if(!this.hass||!this._config.entity)return;const t=this.hass.states[this._config.entity];t&&(this._tasks=bt(t))}render(){try{if(!this.hass)return V`<div>Loading hass...</div>`;if(!this._config)return V`<div>Loading config...</div>`;const t=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.clickup_status?.status&&e.set(t.clickup_status.status,t.clickup_status.status)}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],e=this._tasks.length>0?function(t){const e=new Set;return t.forEach(t=>{t.tags?.forEach(t=>{t.name&&e.add(t.name)})}),Array.from(e).map(t=>({value:t,label:t})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],i=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.assignees?.forEach(t=>{t.id&&t.username&&e.set(t.id.toString(),t.username)})}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[],s=this._tasks.length>0?function(t){const e=new Map;return t.forEach(t=>{t.custom_fields?.forEach(t=>{t.id&&t.name&&e.set(t.id,t.name)})}),Array.from(e.entries()).map(([t,e])=>({value:t,label:e})).sort((t,e)=>t.label.localeCompare(e.label))}(this._tasks):[];return V`
      <div class="card-config">
        ...
        ${this._config.show_custom_fields&&s.length>0?V`
          <div class="config-section">
        ...
        <div class="config-section">
          ...
          ${"custom_field"===this._config.group_by&&s.length>0?V`
            <ha-selector
              .hass=${this.hass}
              .selector=${{select:{options:s.map(t=>({value:t.value,label:t.label}))}}}
