...
`;async function It(t,e,i){try{return e.clickup_id?(await t.callService("clickup","update_task_status",{task_id:e.clickup_id,status:i}),{success:!0}):{success:!1,error:"Task does not have a ClickUp ID"}}catch(t){return console.error("Error updating task status:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}let Pt=class extends nt{constructor(){super(...arguments),this.value="",this.editing=!1,this.placeholder="Click to edit",this.required=!1,this.multiline=!1,this._tempValue="",this._error=""}static{this.styles=r`
  ...
  `}render(){return B`
    ...
    `}_renderDueDateField(t,e){if(!this._config.show_due_date)return B``;if(!t.due&&!this._config.always_show_fields?.due_date)return B``;const i=this._config.field_icons?.due_date,a=i||"mdi:calendar-end",s=this._config.field_styles?.due_date||"";return B`
      <span class="date-item due-date ${t.due?zt(t.due):""}" style="${e} ${s}">
        <ha-icon icon="${a}"></ha-icon>
        ${t.due?At(t.due):"—"}
      </span>
    ...
    `}_renderPriorityField(t,e){if(!this._config.show_priority)return B``;if(!(null!==t.priority&&void 0!==t.priority)&&!this._config.always_show_fields?.priority)return B``;const i=t.priority??null,a=this._config.field_icons?.priority,s=a||vt[i]||vt.null,o=bt[i]||bt.null,r=this._config.field_styles?.priority||"";return B`
      <ha-icon
        ...
        style="color: ${o}; ${e} ${r}"
      ></ha-icon>
    `}_renderSingleCustomField(t,e,i,a){if(!t.custom_fields||0===t.custom_fields.length)return B``;const s=t.custom_fields.find(t=>t.id===e);if(!s)return B``;const o=this._config.field_styles?.custom_fields?.[s.name]||"",r=this._config.field_icons?.custom_fields?.[s.name],n=s.name.replace(/[^a-zA-Z0-9]/g,"-"),l=a||s.name;return B`
      <div class="custom-field" data-field="${n}" style="${i} ${o}">
        ${r?B`<ha-icon icon="${r}"></ha-icon>`:""}
        <span class="field-name">${l}:</span>
        <span class="field-value">${Dt(s)}</span>
      </div>
    ...
    `}_renderFieldCustomization(){const t=this._config.field_styles||{},e=this._config.field_icons||{},i=this._getVisibleFieldTypes();if(0===i.length)return B`
      ...
      `;const a=[{key:"priority",label:"Priority",defaultIcon:"mdi:flag"},{key:"status",label:"Status",defaultIcon:"mdi:circle"},{key:"start_date",label:"Start Date",defaultIcon:"mdi:calendar-start"},{key:"due_date",label:"Due Date",defaultIcon:"mdi:calendar-end"},{key:"dates",label:"Dates (Start + Due)",defaultIcon:"mdi:calendar-range"},{key:"tags",label:"Tags",defaultIcon:"mdi:tag-multiple"},{key:"assignees",label:"Assignees",defaultIcon:"mdi:account-multiple"},{key:"location",label:"Location",defaultIcon:"mdi:folder-outline"}],s=this._tasks.length>0?wt(this._tasks):[],o=[];return i.forEach(t=>{if(t.startsWith("custom_field:")){const e=t.replace("custom_field:",""),i=s.find(t=>t.value===e);i&&o.push({key:`custom_fields.${e}`,label:`Custom: ${i.label}`,defaultIcon:"mdi:form-textbox",isCustomField:!0})}else{const e=a.find(e=>e.key===t);e&&o.push(e)}}),B`
