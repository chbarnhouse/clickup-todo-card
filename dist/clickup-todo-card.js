...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.44  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class Ct extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1,this._statusDropdownTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Et}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return V`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return V`
        ...
        `;this._tasks=bt(t);const e=$t(function(t,e){let i=[...t];if(!e.filters)return i;if(e.filters.status?.length&&(i=i.filter(t=>{const i=t.clickup_status?.status;return i&&e.filters.status.includes(i)})),e.filters.priority?.length&&(i=i.filter(t=>{const i=null===t.priority?null:t.priority;return e.filters.priority.includes(i)})),e.filters.tags?.length&&(i=i.filter(t=>!!t.tags?.length&&t.tags.some(t=>e.filters.tags.includes(t.name)))),e.filters.assignees?.length&&(i=i.filter(t=>!!t.assignees?.length&&t.assignees.some(t=>e.filters.assignees.includes(t.id.toString())))),e.filters.due_date_range){const{start:t,end:s}=e.filters.due_date_range;i=i.filter(e=>{if(!e.due)return!1;const i=new Date(e.due).getTime();return!(t&&i<new Date(t).getTime())&&!(s&&i>new Date(s).getTime())})}return i}(this._tasks,this._config),this._config),i=kt(e,this._config.group_by||"none",this._config.group_field_id),s=!1!==this._config.add_button_overlay,a=this._config.add_button_position||"bottom-right",o=!s&&a.startsWith("top"),r=!s&&a.startsWith("bottom");return V`
        <ha-card>
    ...
    `}async _toggleTask(t){const e="completed"===t.status?"needs_action":"completed";try{await this.hass.callService("todo","update_item",{entity_id:this._config.entity,item:t.uid,status:e})}catch(t){console.error("Error toggling task:",t)}}_toggleStatusDropdown(t){this._statusDropdownTask=this._statusDropdownTask===t?null:t}_renderStatusDropdown(t){const e=this.hass.states[this._config.entity],i=e?.attributes?.available_statuses||[];console.log("[ClickUp Card] Available statuses from entity:",i),console.log("[ClickUp Card] Entity attributes:",e?.attributes);let s=[];if(i.length>0)s=i.map(t=>({name:t.status,color:t.color||"#d3d3d3",type:t.type})),console.log("[ClickUp Card] Using entity statuses:",s);else{const t=yt(this._tasks);t.length>0?(s=t,console.log("[ClickUp Card] Using task statuses:",s)):(s=[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}],console.log("[ClickUp Card] Using fallback statuses:",s))}return V`
    ...
    `}}t([ht({attribute:!1})],St.prototype,"hass",void 0),t([ut()],St.prototype,"_config",void 0),t([ut()],St.prototype,"_tasks",void 0),t([ut()],St.prototype,"_helpers",void 0),customElements.get("clickup-todo-card-editor")||(customElements.define("clickup-todo-card-editor",St),console.log("ClickUp Todo Card Editor: Manually registered custom element"));var Et=Object.freeze({__proto__:null,ClickUpTodoCardEditor:St});export{Ct as ClickUpTodoCard};
