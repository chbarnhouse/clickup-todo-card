...
`;async function zt(t,e,i){try{return e.clickup_id?(await t.callService("clickup","update_task_status",{task_id:e.clickup_id,status:i}),{success:!0}):{success:!1,error:"Task does not have a ClickUp ID"}}catch(t){return console.error("Error updating task status:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}let Ot=class extends nt{constructor(){super(...arguments),this.value="",this.editing=!1,this.placeholder="Click to edit",this.required=!1,this.multiline=!1,this._tempValue="",this._error=""}static{this.styles=r`
  ...
  `}render(){return j`
    ...
    `}_renderDisplay(){const t=Pt.find(t=>t.value===this.value)||Pt[4];return j`
      <div
    ...
    `}_toggleDropdown(){this._isOpen=!this._isOpen,this._isOpen&&setTimeout(()=>{const t=e=>{this.shadowRoot?.contains(e.target)||(this._isOpen=!1,document.removeEventListener("click",t))};document.addEventListener("click",t)},0)}_selectPriority(t){const e=this.value;e!==t&&(this.value=t,this.dispatchEvent(new CustomEvent("value-changed",{detail:{value:t,oldValue:e},bubbles:!0,composed:!0}))),this._isOpen=!1}connectedCallback(){super.connectedCallback(),this._handleKeyDown=this._handleKeyDown.bind(this),document.addEventListener("keydown",this._handleKeyDown)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this._handleKeyDown)}_handleKeyDown(t){"Escape"===t.key&&this._isOpen&&(this._isOpen=!1)}};t([pt({type:Number})],Ft.prototype,"value",void 0),t([pt({type:Boolean})],Ft.prototype,"showLabel",void 0),t([pt({type:Boolean})],Ft.prototype,"compact",void 0),t([ut()],Ft.prototype,"_isOpen",void 0),Ft=t([ct("editable-priority")],Ft);let Mt=class extends nt{constructor(){super(...arguments),this.value=null,this.label="Date",this.icon="mdi:calendar",this.compact=!1,this.allowNull=!0,this.dateType="due",this._editing=!1,this._tempValue=""}static{this.styles=r`
    :host {
      display: inline-block;
      position: relative;
    }
  ...
  `}render(){return j`
    ...
    `}_openDialog(){this._isOpen=!0}_closeDialog(){this._isOpen=!1}_selectStatus(t){const e=this.value;(e?.name||e?.status||"")!==(t.name||t.status||"")&&(this.value=t,this.dispatchEvent(new CustomEvent("value-changed",{detail:{value:t,oldValue:e},bubbles:!0,composed:!0}))),this._isOpen=!1}};t([pt({type:Object})],It.prototype,"value",void 0),t([pt({type:Array})],It.prototype,"options",void 0),t([pt({type:Boolean})],It.prototype,"compact",void 0),t([ut()],It.prototype,"_isOpen",void 0),It=t([ct("editable-status")],It),console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 2.2.0  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class Lt extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...yt},this._tasks=[],this._displayedTasks=[],this._editingTask=null,this._showAddDialog=!1,this._selectedTasks=new Set,this._selectionMode=!1,this._draggedTask=null,this._dragOverTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Rt}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...yt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...yt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var a=e.get("hass");return!a||a.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return j`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return j`
        ...
        `;this._tasks=$t(t);let e=this._tasks;if("custom"===this._config.sort_by){const t=function(t){try{const e=Et(t),i=localStorage.getItem(e);return i?JSON.parse(i):null}catch(t){return console.error("Error loading custom order:",t),null}}(this._config.entity);t&&(e=function(t,e){const i=new Map(t.map(t=>[t.uid,t])),a=[],s=new Set;for(const t of e){const e=i.get(t);e&&(a.push(e),s.add(t))}for(const e of t)s.has(e.uid)||a.push(e);return a}(this._tasks,t))}const i=St(function(t,e){let i=[...t];if(!e.filters)return i;if(e.filters.status?.length&&(i=i.filter(t=>{const i=t.clickup_status?.status;return i&&e.filters.status.includes(i)})),e.filters.priority?.length&&(i=i.filter(t=>{const i=null===t.priority?null:t.priority;return e.filters.priority.includes(i)})),e.filters.tags?.length&&(i=i.filter(t=>!!t.tags?.length&&t.tags.some(t=>e.filters.tags.includes(t.name)))),e.filters.assignees?.length&&(i=i.filter(t=>!!t.assignees?.length&&t.assignees.some(t=>e.filters.assignees.includes(t.id.toString())))),e.filters.due_date_range){const{start:t,end:a}=e.filters.due_date_range;i=i.filter(e=>{if(!e.due)return!1;const i=new Date(e.due).getTime();return!(t&&i<new Date(t).getTime())&&!(a&&i>new Date(a).getTime())})}return i}(e,this._config),this._config);this._displayedTasks=i;const a=Ct(i,this._config.group_by||"none",this._config.group_field_id),s=!1!==this._config.add_button_overlay,o=this._config.add_button_position||"bottom-right",r=!s&&o.startsWith("top"),n=!s&&o.startsWith("bottom");return j`
        <ha-card>
    ...
    `}_renderDates(t){const e=this._config.show_start_date&&t.start_date,i=this._config.show_due_date&&t.due;return e||i?j`
      <div class="task-dates">
        ...
        ${i?j`
          <span class="date-item due-date ${function(t){if(!t)return"";let e;if(e="number"==typeof t||"string"==typeof t?new Date(t):t,isNaN(e.getTime()))return"";const i=new Date,a=new Date(i.getFullYear(),i.getMonth(),i.getDate()),s=new Date(e.getFullYear(),e.getMonth(),e.getDate()).getTime()-a.getTime(),o=Math.ceil(s/864e5);return o<0?"overdue":0===o?"today":1===o?"tomorrow":o<=7?"this-week":"future"}(t.due)}">
    ...
    `}_renderStartDateField(t,e){return this._config.show_start_date?j`
      <editable-date
        ...
        @click=${t=>t.stopPropagation()}
      ></editable-date>
    `:j``}_renderDueDateField(t,e){return this._config.show_due_date?j`
      <editable-date
        class="task-due-date"
        style="${e}"
        .value=${t.due||null}
    ...
    `:j``}_renderTagsField(t,e){return this._config.show_tags?j`
      <editable-tags
        style="${e}"
        .value=${t.tags||[]}
        .compact=${this._config.compact_mode}
        ...
        @click=${t=>t.stopPropagation()}
      ></editable-tags>
    `:j``}_renderAssigneesField(t,e){return this._config.show_assignees?j`
      <editable-assignees
        style="${e}"
        .value=${t.assignees||[]}
        ...
        @click=${t=>t.stopPropagation()}
      ></editable-assignees>
    `:j``}_renderCustomFieldsField(t,e){if(!this._config.show_custom_fields)return j``;const i=this._renderCustomFields(t);return i&&0!==i.values.length?j`<div style="${e}">${i}</div>`:j``}_renderStatusField(t,e){if(!this._config.show_status)return j``;const i=wt(t),a=t.clickup_status||{status:"to do",type:"open",color:"#d3d3d3"};return j`
      <editable-status
        style="${e}"
        .value=${{name:a.status,color:a.color||"#d3d3d3",type:a.type||"open"}}
        .options=${i.map(t=>({status:t.name,color:t.color,type:t.type}))}
        .compact=${this._config.compact_mode}
        @value-changed=${e=>this._handleStatusChange(t,e.detail.value)}
        @click=${t=>t.stopPropagation()}
      ></editable-status>
    `}_renderPriorityField(t,e){return this._config.show_priority&&null!==t.priority&&void 0!==t.priority?j`
      <editable-priority
        style="${e}"
        .value=${t.priority}
        .compact=${this._config.compact_mode}
        ...
        @click=${t=>t.stopPropagation()}
      ></editable-priority>
    `:j``}_renderTaskLocation(t){if(!this._config.show_task_locations)return j``;const e=[];return(t.space_info?.name||t.space?.name)&&e.push(t.space_info?.name||t.space.name),t.folder_info?.name&&e.push(t.folder_info.name),(t.list_info?.name||t.list?.name)&&e.push(t.list_info?.name||t.list.name),0===e.length?j``:j`
    ...
    `}catch(t){return console.error("Error rendering ClickUp Todo Card editor:",t),j`
      ...
      `}}_handleTabChanged(t){this._selectedTab=t.detail.index}_renderConfigTab(){return j`
      <div class="tab-panel">
    ...
    `}_renderVisibilityTab(t){return j`
      <div class="tab-panel">
        <div class="config-section">
          <h3>Header</h3>
      ...
      <div class="tab-panel">
        ...
        <div class="config-section">
          ...
          <ha-formfield .label=${"Use Custom Grid Layout"}>
            <ha-switch
              .checked=${void 0!==this._config.metadata_grid}
              @change=${this._toggleMetadataGrid}
            ></ha-switch>
          </ha-formfield>

          ${this._config.metadata_grid?j`
    ...
    `}}t([pt({attribute:!1})],Vt.prototype,"hass",void 0),t([ut()],Vt.prototype,"_config",void 0),t([ut()],Vt.prototype,"_tasks",void 0),t([ut()],Vt.prototype,"_helpers",void 0),t([ut()],Vt.prototype,"_selectedTab",void 0),customElements.get("clickup-todo-card-editor")||(customElements.define("clickup-todo-card-editor",Vt),console.log("ClickUp Todo Card Editor: Manually registered custom element"));var Rt=Object.freeze({__proto__:null,ClickUpTodoCardEditor:Vt});export{Lt as ClickUpTodoCard};
