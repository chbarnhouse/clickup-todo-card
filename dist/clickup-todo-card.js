...
`;console.info("%c  CLICKUP-TODO-CARD  \n%c  Version 1.0.46  ","color: orange; font-weight: bold; background: black","color: white; font-weight: bold; background: dimgray");class Ct extends nt{constructor(){super(...arguments),this._config={type:"custom:clickup-todo-card",entity:"",...vt},this._tasks=[],this._editingTask=null,this._showAddDialog=!1,this._statusDropdownTask=null}get config(){return this._config}static async getConfigElement(){return await Promise.resolve().then(function(){return Et}),document.createElement("clickup-todo-card-editor")}static getStubConfig(){return{type:"custom:clickup-todo-card",entity:"",...vt}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={...vt,...t}}getCardSize(){return 3+(this._tasks?.length||0)}shouldUpdate(t){if(!this._config||!this.hass)return!0;if(!this._config.entity)return!0;try{return function(t,e,i){if(e.has("config")||i)return!0;if(t.config.entity){var s=e.get("hass");return!s||s.states[t.config.entity]!==t.hass.states[t.config.entity]}return!1}(this,t,!1)}catch(t){return console.error("Error in shouldUpdate:",t),!0}}render(){try{if(!this._config||!this.hass)return B`<ha-card><div class="warning">Loading...</div></ha-card>`;if(!this._config.entity)return B`
        ...
        <ha-card>
          ...
          ${o?B`<div class="button-container">${this._renderFloatingAddButton(t)}</div>`:""}
      ...
      `}}_renderHeader(t){if(this._config.hide_header)return B``;const e=this._config.title||t.attributes.friendly_name||"Tasks",i=!1!==this._config.show_task_count,s=!0!==this._config.hide_title;if(!s&&!i)return B``;const a=this._tasks.length;return B`
      <div class="card-header">
        ${s?B`<div class="name">${e}</div>`:""}
        ${i?B`<div class="task-count ${a<10?"single-digit":""}">${a}</div>`:""}
      </div>
    `}_renderFloatingAddButton(t){const e="unavailable"===t.state,i=void 0!==this._config.add_button_text?this._config.add_button_text:"Add Task",s=this._config.add_button_position||"bottom-right",a=!1!==this._config.add_button_overlay;return B`
      <button
    ...
    `:B`
      ...
      `}_renderTask(t){const e=function(t){if(!t.due)return!1;const e=new Date;return new Date(t.due)<e&&"completed"!==t.status}(t),i="completed"===t.status,s=this._config.show_status&&t.clickup_status,a=this._statusDropdownTask===t.uid;return B`
    ...
    `}async _changeTaskStatus(t,e){try{this._statusDropdownTask=null,await this.hass.callService("clickup","update_task_status",{task_id:t.clickup_id,status:e})}catch(t){console.error("Error changing task status:",t)}}_openAddDialog(){this._showAddDialog=!0}_openEditDialog(t){this._editingTask=t}_renderAddDialog(){const t=yt(this._tasks),e=t.length>0?t:[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}];return B`
      <ha-dialog
        open
    ...
    `}_renderEditDialog(){if(!this._editingTask)return B``;const t=this._editingTask,e=t.start_date?new Date("number"==typeof t.start_date?t.start_date:parseInt(t.start_date)).toISOString().split("T")[0]:"",i=t.due?new Date(t.due).toISOString().split("T")[0]:"",s=yt(this._tasks),a=s.length>0?s:[{name:"TO DO",color:"#d3d3d3",type:"open"},{name:"IN PROGRESS",color:"#4194f6",type:"custom"},{name:"IN REVIEW",color:"#f6c342",type:"custom"},{name:"COMPLETE",color:"#6bc950",type:"closed"},{name:"BLOCKED",color:"#f50000",type:"custom"}];return B`
      <ha-dialog
        open
